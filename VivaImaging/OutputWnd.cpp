...
#else
  m_wndOutputBuild.SetWindowPos(nullptr, -1, -1, cx, cy, SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOZORDER);
#endif
}
...
void COutputWnd::AppendOutputString(LPTSTR msg)
{
  m_wndOutputBuild.AddString(msg);
  if (IsWindowVisible())
    m_wndOutputBuild.PostMessage(WM_VSCROLL, MAKEWPARAM(SB_BOTTOM, 0), 0);
}

BOOL COutputWnd::LoadState(LPCTSTR lpszProfileName, int nIndex, UINT uiID)
{
  BOOL rtn = CDockablePane::LoadState(lpszProfileName, nIndex, uiID);
