...
int CFileView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  if (!m_thumbView.Create(dwViewStyle, rectDummy, this, 101))
  {
    TRACE0("ThumbnailView를 만들지 못했습니다.\n");
    return -1;
  }
...
#else
  m_thumbView.SetWindowPos(NULL, rectClient.left + 1, rectClient.top + 1, rectClient.Width() - 2, rectClient.Height() - 2, SWP_NOACTIVATE | SWP_NOZORDER);
#endif
}
...
void CFileView::OnPaint()
{
	...
	CRect rectTree;
  m_thumbView.GetWindowRect(rectTree);
...
void CFileView::UpdateThumbView(KIntVector& pageIdList, int flag)
{
  m_thumbView.UpdateThumbView(pageIdList, flag);
}
void CFileView::UpdateThumbView(BOOL updateAll, int index, int flag)
{
  m_thumbView.UpdateThumbView(updateAll, index, flag);
}

void CFileView::OnActivePageChanged(int flag)
{
  m_thumbView.OnActivePageChanged(flag);
  /*
  if (flag & CHANGED_NUMBER_OF_PAGES)
  {
    m_thumbView.RecalcScrollRange();
  }
  m_thumbView.Invalidate(TRUE);
  */
}
