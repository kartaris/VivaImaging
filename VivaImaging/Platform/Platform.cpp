...
BOOL KDeleteFile(LPCTSTR fileName)
{
  ...
  if (n_len >= MAX_PATH)
  {
    StringCchCopy(ext_filename, KMAX_EXTPATH, UNC_FILE_PREFIX);
    StringCchCat(ext_filename, KMAX_EXTPATH, fileName);
...
{
  TCHAR ext_prev[KMAX_EXTPATH];
  TCHAR ext_new[KMAX_EXTPATH];

  if (lstrlen(prevFileName) >= MAX_PATH)
  {
    StringCchCopy(ext_prev, KMAX_EXTPATH, UNC_FILE_PREFIX);
    StringCchCat(ext_prev, KMAX_EXTPATH, prevFileName);
    prevFileName = ext_prev;
  }
  if (lstrlen(newFileName) >= MAX_PATH)
  {
    StringCchCopy(ext_new, KMAX_EXTPATH, UNC_FILE_PREFIX);
    StringCchCat(ext_new, KMAX_EXTPATH, newFileName);
    newFileName = ext_new;
  }
...
HANDLE OpenLogFileShared(LPCTSTR fileName)
{
...
{
  if (lstrlen(filename) >= KMAX_FILENAME)
    return FALSE;
